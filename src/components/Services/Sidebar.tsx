...
const SideLayout: React.FC = () => {
  ...
  const handleButtonClick = (buttonName: string) => {
    if(buttonName == 'employees' || buttonName === 'inventory'){
        ...
        if (buttonName === 'employees') {
          return (
            <div className="flex flex-col gap-2 ml-4 mt-2 border-l-2 border-gray-300 pl-4">
              <button className="py-2 px-4 text-left text-sm hover:bg-gray-200 rounded" onClick={() => navigate('/employees/attendance')}>
                <p className="font-poppins text-[12px] text-left">Attendance & Leave Requests</p>
              </button>
              <button className="py-2 px-4 text-left text-sm hover:bg-gray-200 rounded" onClick={() => navigate('/employees/reports')}>
                <p className="font-poppins text-[12px] text-left">Hospital Reports</p>
              </button>
              <button className="py-2 px-4 text-left text-sm hover:bg-gray-200 rounded" onClick={() => navigate('/employees/payrolls')}>
          ...
          return (
            <div className="flex flex-col gap-2 ml-4 mt-2 border-l-2 border-gray-300 pl-4">
              <button className="py-2 px-4 text-left text-sm hover:bg-gray-200 rounded" onClick={() => navigate('/inventory/items')}>
                <p className="font-poppins text-[12px] text-left">Inventory Items</p>
              </button>
              <button className="py-2 px-4 text-left text-sm hover:bg-gray-200 rounded" onClick={() => navigate('/inventory/purchase-orders')}>
  ...
  const renderButton = (buttonName: string, icon: string, activeIcon: string, label: string, hasDropdown: boolean = false) => (
    <>
      <button
      ...
      >
        ...
        <div className="flex gap-4 items-center ">
          <p className={`${activeButton === buttonName ? 'text-lightblue' : ''} font-poppins text-[15px] font-medium leading-[15.4px] text-left`}>
            {label}
          </p>
          {hasDropdown && (
            <img
